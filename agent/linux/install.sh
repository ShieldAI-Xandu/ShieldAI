...
# 2. Write config (server URL + one-time enrollment token), root-only. Also
# clear any agent.json from a prior install — otherwise agent-run.sh finds an
# existing (possibly stale/revoked) agent token and never uses the fresh
# enrollment token we just wrote, so a reinstall silently fails to re-enroll.
mkdir -p "$DATA_DIR"
