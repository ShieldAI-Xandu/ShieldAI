...
async function buildWindowsInstaller({ serverUrl, enrollmentToken, intervalMinutes }) {
  ...
  const lines = [
    "<#",
    "  ShieldAI vCISO Windows Agent Installer — self-contained, pre-configured for this endpoint.",
    ...
    "# Clear any agent.json from a prior install — otherwise agent-run.ps1 finds",
    "# an existing (possibly stale/revoked) agent token and never uses the fresh",
    "# enrollment token below, so a reinstall silently fails to re-enroll. It's",
    "# locked read-only (SYSTEM/Administrators, no delete) once enrolled, so a",
    "# plain Remove-Item silently no-ops on the ACL — take ownership and reset",
    "# it first so the delete actually goes through.",
...
async function buildUnixInstaller(os, { serverUrl, enrollmentToken, intervalMinutes }) {
  ...
  const lines = [
    ...
    "",
    'mkdir -p "$DATA_DIR"',
    "# Clear any agent.json from a prior install — otherwise agent-run.sh finds",
    "# an existing (possibly stale/revoked) agent token and never uses the fresh",
    "# enrollment token below, so a reinstall silently fails to re-enroll.",
