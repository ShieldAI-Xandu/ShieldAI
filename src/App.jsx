...
//  provider.
//
//  AI_MODELS below is real routing/debugging info (Claude/Gemini/GPT-4o).
//  Direction as of 2026-07-24: the public marketing/landing pages (Marketing-
//  Page, Landing) are a deliberate exception and DO name the vendors — see
//  the "Meet Mastermind" section in MarketingPage for the rationale. Every
//  OTHER surface — i.e. anywhere inside the actual product once someone has
//  started an assessment or logged in (IntakeChat, AnalysisScreen, Dashboard,
//  PolicyLibrarySection, etc.) — never names a vendor to the customer. Those
//  surfaces use AIChip (generic "🧠 Mastermind" only) or, where a 3-way
//  breakdown is useful for UX, MASTERMIND_ENGINES below — role labels with no
//  vendor name attached, ever.
// ─────────────────────────────────────────────────────────────
const AI_MODELS = {
  claude: {
    id: "claude",
...
// Customer-facing engine roles — no vendor name, ever. Used anywhere the UI
// wants to show "which part of Mastermind is working" without disclosing
// which underlying model actually does that work.
const MASTERMIND_ENGINES = {
  analyst:  { id: "analyst",  icon: "🧠", label: "Analyst",      color: C.purple,
              specialty: "Risk analysis, policy drafting, compliance reasoning" },
  intel:    { id: "intel",    icon: "🔍", label: "Intelligence", color: C.accent,
              specialty: "Threat intelligence, real-time research" },
  advisory: { id: "advisory", icon: "📋", label: "Advisory",     color: C.green,
...
// Paid-feature teaser for Free tier: shows what's inside (never the real
// data) with an upgrade CTA, instead of either hiding the nav tab entirely or
// silently rendering empty/broken. Free tier gets exactly one thing — the
// initial posture score on Overview — everything else routes here.
function LockedFeature({ icon, title, blurb, points, capability }) {
  return (
    <div style={{maxWidth:560,margin:"40px auto",textAlign:"center"}}>
      <div style={{width:56,height:56,borderRadius:14,margin:"0 auto 18px",display:"flex",
        alignItems:"center",justifyContent:"center",background:`${C.purple}18`,
        border:`1px solid ${C.purple}44`,fontSize:26}}>{icon || "🔒"}</div>
      <h2 style={{color:C.text,fontSize:20,fontWeight:700,margin:"0 0 8px"}}>{title}</h2>
      <p style={{color:C.textSec,fontSize:14,lineHeight:1.6,margin:"0 0 20px"}}>{blurb}</p>
      {points?.length > 0 && (
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24,textAlign:"left"}}>
          {points.map((p,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"10px 14px",
              background:C.surface,border:`1px solid ${C.border}`,borderRadius:8}}>
              <span style={{color:C.textMut,fontSize:13}}>🔒</span>
              <span style={{color:C.textSec,fontSize:13}}>{p}</span>
            </div>
          ))}
        </div>
      )}
      <button onClick={() => showUpgradePrompt({
          error: "This feature isn't included on your current plan.",
          code: "UPGRADE_REQUIRED", capability, currentTier: "free",
        })}
...
function AIChip({ model }) {
  return (
    <span title="Generated by ShieldAI Mastermind"
      style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",
      borderRadius:20,background:`${C.purple}15`,border:`1px solid ${C.purple}33`,
...
function IntakeChat({ onComplete }) {
    ...
    if (text.includes("<ASSESSMENT_DONE>")) {
      ...
      if (match) {
        try {
          ...
          const finalMsg = "Excellent! I have a complete picture of your security landscape. Spinning up the full Mastermind analysis now — risk modeling, threat intelligence, and your executive summary. This takes about 30 seconds...";
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      {/* Mastermind indicator — unified brand only, no vendor names */}
      <div style={{padding:"10px 20px",background:C.surface,borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 10px",
          borderRadius:20,background:`${C.purple}18`,border:`1px solid ${C.purple}44`,
          color:C.purple,fontSize:11,fontWeight:600}}>
          🧠 ShieldAI Mastermind
        </div>
        <div style={{marginLeft:"auto",fontSize:11,color:C.textMut}}>
          Conducting your security intake
        </div>
      </div>
...
function setAuthToken(token) {
  ...
  // Persist across refreshes — without this, any page reload wiped auth
  // entirely (AUTH_TOKEN is just a JS variable, reset on every module load),
  // which is what made refresh look like it "exited the app."
  try {
    if (token) localStorage.setItem("shieldai_token", token);
    else localStorage.removeItem("shieldai_token");
  } catch { /* storage unavailable (e.g. private browsing) — session just won't survive a refresh */ }
}
...
function getAuthToken() {
  if (!AUTH_TOKEN) {
    try { AUTH_TOKEN = localStorage.getItem("shieldai_token"); } catch { /* storage unavailable */ }
  }
  return AUTH_TOKEN;
}

// Persists which "deep" screen the user is on (dashboard or console, plus the
// ids needed to re-fetch it) across a page refresh. sessionStorage, not
// localStorage — this should only survive within the same tab session, not
// follow the user to a new tab or persist after they close the browser.
// Without this, a refresh always restarted at phase "home" (the useState
// default), which is what made refresh look like it kicked you out to the
// homepage even once the auth-token fix kept you logged in.
const RESUME_KEY = "shieldai_resume";
function saveResumeState(state) {
  try { sessionStorage.setItem(RESUME_KEY, JSON.stringify(state)); } catch { /* ignore */ }
}
...
// Proactively show the same upgrade modal a 402 would trigger — for UI that's
// locked up front (so the user never sees a loading spinner just to be told
...
// Free tier's path: buildPrograms is off for Free, so /api/programs/generate
// would 402. Instead, save the assessment and fetch the deterministic,
// zero-AI-cost posture-preview endpoint (see server.js) — this is the "initial
// report" Free tier gets. Shaped to match results.riskOverview exactly, so
// OverviewSection renders it with no changes; every other Dashboard tab just
// sees an empty section and (per the tier gates added below) shows a locked
// upgrade teaser instead of trying to render nothing.
async function generateFreePreview(assessmentData) {
  const saveRes = await authFetch(`${API_BASE}/api/assessments`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(assessmentData),
  ...
  const previewRes = await authFetch(`${API_BASE}/api/assessments/${assessmentId}/posture-preview`);
  if (!previewRes.ok) throw new Error(`Failed to compute your score: ${previewRes.status}`);
  const preview = await previewRes.json();

  return {
    sections: {
      riskOverview: {
        postureScore: preview.postureScore,
        postureLevel: preview.postureLevel,
        executiveSummary: preview.executiveSummary,
        topThreats: [],
        breakdown: {
          methodology: preview.methodology,
          functions: preview.functions,
          weakestAreas: preview.weakestAreas,
          complianceNote: preview.complianceNote,
          frameworkLens: preview.frameworkLens,
          lens: preview.lens,
...
// ─────────────────────────────────────────────────────────────
function AnalysisScreen({ assessment, regenerate, onComplete, freePreview }) {
  ...
  useEffect(() => {
    ...
    const task = freePreview
      ? generateFreePreview(assessment)
      : regenerate
  ...
  // Map pipeline step labels to a customer-facing engine role (no vendor name).
  const stepEngineMap = {
    "Risk overview & top threats": "analyst",
    "Prioritized roadmap & quick wins": "analyst",
    "Core security policies": "analyst",
    "Operational security policies": "analyst",
    "Compliance framework gap analysis": "analyst",
    "Incident response workflows": "analyst",
    "Threat intelligence": "intel",
    "Recommended tool stack": "intel",
    "Awareness training program": "advisory",
    "Executive summary report": "advisory",
  };
  const activeEngine = stepEngineMap[progress.label] || "analyst";
  ...
  // Free tier: no multi-step AI pipeline to show progress for — the posture
  // score is computed instantly. A short, honest loading state instead of the
  // full "10 components" screen, which would misrepresent what's happening.
  if (freePreview) {
    return (
      <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",
        justifyContent:"center",fontFamily:"Inter,system-ui,sans-serif",padding:24}}>
        <div style={{width:"100%",maxWidth:420,textAlign:"center"}}>
          ...
          <p style={{color:C.textSec,fontSize:14}}>
            Computing your NIST/CIS posture score
          </p>
        </div>
        <style>{`
      ...
      <div style={{width:"100%",maxWidth:520,textAlign:"center"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
          {Object.values(MASTERMIND_ENGINES).map(e => (
            <div key={e.id} style={{padding:"12px",background:C.surface,
              border:`1px solid ${activeEngine===e.id?e.color+"55":C.border}`,
              borderRadius:10,textAlign:"center",transition:"border-color 0.3s"}}>
              <div style={{fontSize:20,marginBottom:4}}>{e.icon}</div>
              <div style={{color:e.color,fontSize:11,fontWeight:600}}>{e.label}</div>
              <div style={{color:C.textMut,fontSize:10,marginTop:2}}>
                {activeEngine===e.id?"● Active":"Standby"}
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{flex:1,minWidth:200}}>
            ...
            <div style={{color:C.textMut,fontSize:12}}>
              {canGenerateFull
                ...
                : "Full generation requires the training delivery add-on or a Growth+ plan."}
            </div>
          </div>
          <label style={{display:"flex",alignItems:"center",gap:7,
            cursor:canGenerateFull?"pointer":"not-allowed",color:canGenerateFull?C.textSec:C.textMut,fontSize:12}}>
            <input type="checkbox" checked={includeManager} disabled={!canGenerateFull}
              onChange={e=>setIncludeManager(e.target.checked)} style={{cursor:"inherit"}}/>
            Include manager / owner track
          </label>
          <button
            onClick={canGenerateFull ? buildProgram : () => showUpgradePrompt({
              error: "Full training-program generation isn't included on your current plan.",
              code: "UPGRADE_REQUIRED", capability: "trainingDelivery", currentTier: "starter",
            })}
            disabled={generating}
            style={{padding:"10px 20px",
              background: !canGenerateFull ? C.surface : generating?C.surface:`linear-gradient(135deg,${C.accent},${C.accentDm})`,
              color: !canGenerateFull ? C.textMut : generating?C.textMut:C.bg,
              border: !canGenerateFull ? `1px solid ${C.border}` : "none",
              borderRadius:8,fontSize:13,fontWeight:700,
              cursor:generating?"wait":"pointer"}}>
            {!canGenerateFull ? "🔒 Generate Full Program" : generating ? "Building curriculum…" : "✦ Generate Full Program"}
          </button>
        </div>
        {generating && (
  ...
  return (
    <div>
      ...
      <p style={{color:C.textSec,fontSize:13,marginBottom:16,lineHeight:1.6}}>
        Select a policy below, answer a few quick questions, and ShieldAI Mastermind will
        generate a complete, ready-to-use policy document tailored to your business.
      </p>
...
function Dashboard({ assessment, results, onReset }) {
  ...
  // specific capability — tiers now differ from EACH OTHER, not just free-vs-
  ...
  // when the account's tier doesn't include that specific feature. Copy names
  // what's actually inside so the paywall reads as a feature list, not a wall.
  const lockedSections = {
    ...
    compliance: <LockedFeature icon="✅" title="Compliance Tracking" capability="complianceAccess"
      blurb="Track your real posture against the frameworks you selected — control by control."
      points={["Only the frameworks you chose at intake","Live agent-vs-answer conflict resolution","Audit-ready evidence and citations"]}/>,
    threats: <LockedFeature icon="🔍" title="Threat Intelligence" capability="threatIntel"
      blurb="Live CVE exposure and breach monitoring for your actual environment."
      points={["Real CVE matching via the NVD feed","Dark-web breach exposure (Have I Been Pwned)","Industry-specific threat briefing"]}/>,
    tools: <LockedFeature icon="🔧" title="Recommended Security Tool Stack" capability="buildPrograms"
      blurb="A tool stack matched to your budget and risk profile."
      points={["6 categories: endpoint, email, MFA, backup, and more","Cost tier and implementation difficulty per category","Tailored to your business size and industry"]}/>,
    training: <LockedFeature icon="🎓" title="Security Awareness Training" capability="trainingPlan"
      blurb="A CISA/NIST-aligned training program for your team."
      points={["Modules tailored to your industry","Phishing simulation schedule","Full curriculum builder with quizzes and slides"]}/>,
    trainingmgr: <LockedFeature icon="👥" title="Training Program Management" capability="trainingDelivery"
      blurb="Assign, track, and report on employee training completion."
      points={["Tokenized learner links","Quarterly scheduling","Fleet-wide completion reporting"]}/>,
    report: <LockedFeature icon="📋" title="Executive CISO Report" capability="buildPrograms"
      blurb="A board-ready report — posture, risk, investment, and ROI in one place."
      points={["Executive summary in plain business language","Key findings and next steps","Downloadable, shareable format"]}/>,
    reports: <LockedFeature icon="📑" title="Reports" capability="reportsAccess"
      blurb="Status, compliance, insurance, and legal reports, ready to deliver."
      ...
      <div style={{width:220,flexShrink:0,background:C.surface,borderRight:`1px solid ${C.border}`,
        ...
        <div style={{padding:"12px 14px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:4}}
            ...
            <span style={{fontSize:10,color:C.purple,letterSpacing:1.5,fontWeight:700}}>MASTERMIND</span>
          </div>
...
function MarketingPage({ onEnterApp, onLogin, onStartDemo, onRedeemCode, onOpenInvestor,
  ...
  return (
    <div className="mkt-root" style={{background:deep,color:ink,fontFamily:"Inter,system-ui,sans-serif",minHeight:"100vh"}}>
        ...
        <Section>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14}}>
            {[
              ...
              { icon:"📧", t:"Email & domain security",
                d:"Free SPF/DKIM/DMARC scanning for your domain — tells you the exact DNS record to add to stop the spoofing attacks that actually hit small businesses." },
              { icon:"🎣", t:"Phishing simulation",
      ...
      {/* Vendor names ARE shown here intentionally. The marketing/landing
          pages are prospect-facing, pre-login pages where naming Claude,
        ...
        <Section>
          <div style={{display:"flex",gap:40,flexWrap:"wrap",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{flex:"1 1 380px",minWidth:280}}>
              ...
              <p style={{fontSize:14.5,color:dim,lineHeight:1.65,maxWidth:480,margin:"0 0 16px"}}>
                Mastermind is ShieldAI's multi-agent AI system, built on Claude, Gemini, and
                ChatGPT — routing each task to whichever one is best suited to it, with
                automatic failover so the platform keeps working if one is unavailable.
                Mastermind drafts, maps, and monitors — but it never acts on your systems,
                and a security engineer reviews what reaches you.
              </p>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <span style={{padding:"5px 12px",borderRadius:20,background:`${C.green}14`,
                  border:`1px solid ${C.green}44`,color:C.green,fontSize:11.5,fontWeight:600}}>AI advises, humans act</span>
                <span style={{padding:"5px 12px",borderRadius:20,background:`${C.green}14`,
                  border:`1px solid ${C.green}44`,color:C.green,fontSize:11.5,fontWeight:600}}>3 industry-leading models</span>
              </div>
            </div>
        ...
        <div style={{maxWidth:760,margin:"0 auto"}}>
          {loading ? <Spinner/> : (
            <>
              ...
              {Object.entries(grouped).map(([fn, questions]) => {
                // Evidence sections carry no NIST weight and deliberately get a
                // neutral color instead of a NIST function color — see the
                // matching comment in ChecklistScreen. NIST_COLORS[fn] is
                // undefined for these section names, which used to silently
                // break the selected-option styling here (answers still saved
                // correctly; the ring/fill just never visibly lit up).
                const isEvidence = questions[0]?.affectsPostureScore === false;
                const color = isEvidence ? C.textSec : NIST_COLORS[fn];
                return (
                <div key={fn} style={{marginBottom:20}}>
                  ...
                  {questions.map(q => (
                    <div key={q.id} style={{marginBottom:14,padding:"14px 16px",background:C.card,
                      border:`1px solid ${answers[q.id] ? color+"44" : C.border}`,borderRadius:12}}>
              ...
              {!showRegenChoice ? (
                <div style={{display:"flex",gap:12,marginTop:8,marginBottom:40}}>
                  <button onClick={saveOnly} disabled={saving || !allAnswered}
                    ...
                    {saving ? "Saving…" : canRegenerate ? "Save Changes Only" : "Save Changes"}
                  </button>
                  {canRegenerate && (
...
function ClientReportsPanel({ clientId }) {
  ...
  const staffTypes = ["compliance", "insurance", "legal", "training"];
...
export default function ShieldAI() {
  ...
  // the last unload, restore the SAME page instead of dropping back to Home —
  // this is the actual fix for refresh exiting to the homepage/logging out.
  useEffect(() => {
    let cancelled = false;
    async function restore() {
      ...
      try {
        ...
        if (resume?.phase === "dashboard" && resume.assessmentId) {
          if (resume.programId) {
            await openProgram(resume.assessmentId, resume.programId);
          } else {
            // Free-tier preview — no program record to fetch; re-run the
            // same zero-cost preview path the assessment originally used.
            const [aRes, previewRes] = await Promise.all([
              authFetch(`${API_BASE}/api/assessments/${resume.assessmentId}`),
              authFetch(`${API_BASE}/api/assessments/${resume.assessmentId}/posture-preview`),
            ]);
            if (aRes.ok && previewRes.ok) {
  ...
  async function openProgram(assessmentId, programId) {
    ...
    saveResumeState({ phase: "dashboard", assessmentId, programId });
    setPhase("dashboard");
  }
  ...
  // above). Kept deliberately brief and unbranded-quiet — this should only
  // ever flash for a moment on a refresh, not read as a "real" loading screen.
  if (restoringSession) {
    return (
  ...
  if (phase === "analysis") {
    return (
      <AnalysisScreen
        ...
        freePreview={!regenAssessmentId && !can("buildPrograms")}
        onComplete={async ({ sections, assessmentId, programId }) => {
