...
import { callAI, callAIText, normalizeProvider, providerStatus } from "./aiProviders.js";
import {
  registerUser,
  loginUser,
  requireAuth,
...
async function callClaudeText({ system, messages, max_tokens, provider }) {
  // Internal pipelines default to Claude. Passing a provider opts into the
  // unified layer (which silently falls back to Claude if that key is absent).
  return await callAIText({ provider, system, messages, max_tokens });
}
...
app.post("/api/claude", requireAuth, async (req, res) => {
  try {
    const { system, messages, max_tokens = 1500, stream = false, provider } = req.body || {};

    // Streaming stays on Claude (the only provider the streaming UI uses).
    // Any non-Claude provider request is served non-streamed via the unified
    // layer, which silently falls back to Claude when a key is missing/failing.
    if (stream && normalizeProvider(provider) === "claude") {
    ...
    // Non-streaming (multi-AI analysis flow): dispatch to the requested engine.
    const result = await callAI({ provider, system, messages, max_tokens });
    // Respond in the Anthropic content-shape the frontend already parses,
    // and surface which engine actually answered (for the UI's model badge).
    res.json({
...
// Which AI providers are configured (admin-visible; no keys exposed).
